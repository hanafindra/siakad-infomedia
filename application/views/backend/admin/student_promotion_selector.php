		...
		<div class="tab-content">
		  	<div class="tab-pane active" id="tab1" style="margin-top: 20px;">
		  		...
		  		<table class="table table-bordered">
					<thead align="center">
						<tr>
							<td align="center"><?php echo get_phrase('nama');?></td >
							<td align="center"><?php echo get_phrase('kelas');?></td >
					...
					?>
						<tr>
							...
							<button type="button" class="btn btn-default"
								...
								<i class="entypo-eye"></i> <?php echo get_phrase('Lihat Rekap Akademik');?>
							</button>
							</td>
							<td>
								<?php if($query->num_rows() < 1):?>
									<select class="form-control selectboxit" name="promotion_status_<?php echo $row['student_id'];?>" style="width: 40px;" id="promotion_status">
										<option value="<?php echo $class_id_to;?>">
											<?php echo get_phrase('pindahkan_ke_kelas') ." - ". $this->crud_model->get_class_name($class_id_to);?>
										</option>
										<option value="<?php echo $class_id_from;?>">
											<?php echo get_phrase('pindahkan_ke_kelas') ." - ". $this->crud_model->get_class_name($class_id_from);?>
									</select>
								<?php endif;?>
								<?php if($query->num_rows() > 0):?>
									<button class="btn btn-success">
										<i class="entypo-check"></i> <?php echo get_phrase('Siswa sudah terdaftar');?>
									</button>
								<?php endif;?>
							</td>
						</tr>
				...
				<center>
					<button type="submit" class="btn btn-success">
						<i class="entypo-check"></i> <?php echo get_phrase('Pindahkan Siswa Yang Terpilih');?>
					</button>
				</center>
