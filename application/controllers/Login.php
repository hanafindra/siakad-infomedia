...
class Login extends CI_Controller {
    ...
    function validate_login() {
      ...
      $credentialst = array('email' => $email, 'password' => sha1($password), 'is_active' => 1);
      ...
      $query = $this->db->get_where('student', $credentialst);
      if ($query->num_rows() > 0) {
          $row = $query->row();
          ...
          $this->session->set_userdata('name', $row->name);
          $this->session->set_userdata('is_terima', $row->is_terima);
      ...
      if($redirect !=''){
        $result = array('status'=>'success', 'redirect'=>site_url().$redirect, 'message'=>'Mohon tunggu...');
      } else {
        $result = array('status'=>'error', 'redirect'=>'', 'message'=>'Akun tidak terdaftar!');
      }
