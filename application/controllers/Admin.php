...
class Admin extends CI_Controller
{
  ...
  function student_profile($student_id)
  {
    ...
    function student_marksheet($student_id = '') {
        ...
        $page_data['page_title'] =   get_phrase('lembari_nilai_untuk') . ' ' . $student_name . ' (' . get_phrase('kelas') . ' ' . $class_name . ')';
        $page_data['student_id'] =   $student_id;
        $page_data['class_id']   =   $class_id;
        $this->load->view('backend/index', $page_data);
    }
    ...
    function student($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            if(html_escape($this->input->post('student_code')) != null){
                ...
                if(!$code_validation) {
                    $this->session->set_flashdata('error_message' , get_phrase('id_ini_tidak_tersedia'));
    ...
    function delete_student($student_id = '', $class_id = '') {
      ...
      $this->session->set_flashdata('flash_message' , get_phrase('siswa_terhapus'));
    ...
    function parent($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            if($validation == 1){
                ...
                $this->session->set_flashdata('error_message' , get_phrase('email_id_ini_tidak_tersedia'));
            }
    ...
    function sections($param1 = '' , $param2 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            if($validation == 1){
                ...
                $this->session->set_flashdata('error_message' , get_phrase('nama_sesi_yang_sama_tidak_diizinkan'));
            }
                ...
                if ($receiver == 'parent') {
                    ...
                    if($parent_id != '' || $parent_id != null) {
                        ...
                        if($receiver_phone == null){
                          $this->session->set_flashdata('error_message' , get_phrase('nomor_wali_murid_tidak_dtemukan'));
                        }
                    }
                }
                $running_year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
    ...
    function class_routine_add()
    {
        ...
        $page_data['page_title'] = get_phrase('tambah_rutinitas_kelas');
        $this->load->view('backend/index', $page_data);
    }
	...
	function manage_attendance2($date='',$month='',$year='',$class_id='' , $section_id = '' , $session = '')
	{
        ...
        $page_data['page_name']  =	'manage_attendance';
        $page_data['page_title'] =	get_phrase('kelola_absensi_harian');
		$this->load->view('backend/index', $page_data);
	}
	function attendance_selector2()
	{
     ...
     function attendance_report() {
         ...
         $page_data['page_title']   = get_phrase('laporan_absensi');
         $this->load->view('backend/index',$page_data);
     }
     function attendance_report_view($class_id = '', $section_id = '', $month = '', $sessional_year = '')
     {
        ...
        $page_data['page_title']        = get_phrase('laporan_absensi_kelas') . ' ' . $class_name . ' : ' . get_phrase('sesi') . ' ' . $section_name;
        $this->load->view('backend/index', $page_data);
     }
    ...
    {   if($this->input->post('class_id') == '' || $this->input->post('sessional_year') == '') {
            $this->session->set_flashdata('error_message' , get_phrase('tolong_pastikan_kelas_dan_tahun_sesi_sudah_dipilih'));
    ...
    function get_invoices() {
        ...
        if(!empty($invoices)) {
            foreach ($invoices as $row) {
                ...
                if ($row->due == 0) {
                    $status = '<button class="btn btn-success btn-xs">'.get_phrase('dibayar').'</button>';
                    $payment_option = '';
                } else {
    ...
    function get_payments() {
        ...
        if(!empty($payments)) {
            foreach ($payments as $row) {
                ...
                $options = '<a href="#" onclick="invoice_view_modal('.$row->invoice_id.')"><i class="entypo-credit-card"></i>&nbsp;'.get_phrase('lihat_fatur').'</a>';
    ...
    function noticeboard_edit($notice_id) {
      ...
      $page_data['page_title'] = get_phrase('edit_pengumuman');
      $this->load->view('backend/index', $page_data);
    }
        ...
        if ($param1 == 'send_new') {
            ...
            if ($_FILES['attached_file_on_messaging']['name'] != "") {
              if($_FILES['attached_file_on_messaging']['size'] > $max_size){
                $this->session->set_flashdata('error_message' , get_phrase('ukuran_file_tidak_boleh_lebih_dari_2_megabyte'));
    ...
    function group_message($param1 = "group_message_home", $param2 = ""){
      ...
      $page_data['page_title']                = get_phrase('olah_pesan_grup');
      $this->load->view('backend/index', $page_data);
    }
    ...
    function system_settings($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'change_skin') {
            ...
            $this->db->update('settings' , $data);
            $this->session->set_flashdata('flash_message' , get_phrase('tema_terpilih'));
            redirect(site_url('admin/system_settings'), 'refresh');
        }
    ...
    function frontend_events($param1 = '', $param2 = '') {
      ...
      if ($param1 == 'edit_event') {
        ...
        $this->session->set_flashdata('flash_message' , get_phrase('kegiatan_berhasil_diperbarui'));
        redirect(site_url('admin/frontend_pages/events'), 'refresh');
      }
      if ($param1 == 'delete') {
    ...
    function frontend_gallery($param1 = '', $param2 = '', $param3 = '') {
      ...
      if ($param1 == 'upload_images') {
        ...
        $this->session->set_flashdata('flash_message' , get_phrase('foto_diunggah'));
      ...
      if ($param1 == 'delete_image') {
        ...
        $this->session->set_flashdata('flash_message' , get_phrase('foto_dihapus'));
    ...
    function frontend_news($param1 = '', $param2 = '') {
      if ($param1 == 'add_news') {
        ...
        $this->session->set_flashdata('flash_message' , get_phrase('berita_berhasil_ditambahkan'));
      ...
      if ($param1 == 'delete') {
        ...
        $this->session->set_flashdata('flash_message' , get_phrase('berita_terhapus'));
        redirect(site_url('admin/frontend_pages/news'), 'refresh');
      }
    }
    ...
    function frontend_settings($task) {
      if ($task == 'update_terms_conditions') {
        ...
        $this->session->set_flashdata('flash_message' , get_phrase('ketentuan_diperbarui'));
      ...
      if ($task == 'update_about_us') {
        ...
        $this->session->set_flashdata('flash_message' , get_phrase('tentang_kami_diperbarui'));
      ...
      if ($task == 'update_privacy_policy') {
        ...
        $this->session->set_flashdata('flash_message' , get_phrase('kebijakan_privasi_diperbarui'));
      ...
      if ($task == 'update_general_settings') {
        ...
        $this->session->set_flashdata('flash_message' , get_phrase('pengaturan_umum_diperbarui'));
      ...
      if ($task == 'update_slider_images') {
        ...
        $this->session->set_flashdata('flash_message' , get_phrase('gambar_slider_diperbarui'));
    ...
    function frontend_themes() {
      ...
      $page_data['page_title']  = get_phrase('tema');
      $this->load->view('backend/index', $page_data);
    }
    ...
    function change_session()
    {
        ...
        $this->db->where('type' , 'running_year');
        $this->db->update('settings' , $data);
        $this->session->set_flashdata('flash_message' , get_phrase('sesi_berubah'));
        redirect(site_url('admin/dashboard'), 'refresh');
    }
