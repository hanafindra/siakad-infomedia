...
class Student extends CI_Controller
{
    ...
    function manage_profile($param1 = '', $param2 = '', $param3 = '')
    {
      $id = $this->session->userdata('student_id');
        if ($this->session->userdata('student_login') != 1) redirect(site_url('login'), 'refresh');
        if ($param1 == 'update_profile_info') {
            $data = array(
              ...
              'tinggi_badan' => html_escape($this->input->post('tb')),
              'berat_badan'  => html_escape($this->input->post('bb')),
              'anak_ke' => html_escape($this->input->post('anak_ke')),
              'jumlah_saudara'  => html_escape($this->input->post('jumlah_saudara')),
              'nis'  => html_escape($this->input->post('nis')),
              // 'nama_ayah' => html_escape($this->input->post('nama_ayah')),
              // 'lahir_ayah' => html_escape($this->input->post('lahir_ayah')),
              // 'pendidikan_ayah'  => html_escape($this->input->post('pendidikan_ayah')),
              // 'pekerjaan_ayah' => html_escape($this->input->post('pekerjaan_ayah')),
              // 'penghasilan_ayah'  => html_escape($this->input->post('penghasilan_ayah')),
              // 'nama_ibu' => html_escape($this->input->post('nama_ibu')),
              // 'lahir_ibu' => html_escape($this->input->post('lahir_ibu')),
              // 'pendidikan_ibu'  => html_escape($this->input->post('pendidikan_ibu')),
              // 'pekerjaan_ibu' => html_escape($this->input->post('pekerjaan_ibu')),
              // 'penghasilan_ibu'  => html_escape($this->input->post('penghasilan_ibu'))
            );

            $validation = email_validation_for_edit($data['email'], $id, 'student');
            if($validation == 1){
                $this->db->where('student_id', $id);
                $this->db->update('student', $data);
                // $str_qry = "call sp_save_data_diri(".$id.",'".json_encode($data)."')";
        				// $this->db->trans_start();
        				// $result = $this->db->query($str_qry);
                ...
                $result = array('status'=>'success', 'message'=>'Simpan data berhasil');
            }
            else{
                // $this->session->set_flashdata('error_message', get_phrase('this_email_id_is_not_available'));
                $result = array('status'=>'error', 'message'=>'Gagal menyimpan data');
            }
        ...
        $page_data['page_title'] = get_phrase('Data Diri');
